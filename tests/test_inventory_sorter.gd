...
func test_inventory_sorter() -> void:
	...
	var sorted_by_type: Array[GameplayItemInstance] = sorter.sort_by_type(items)
	...
	items2.append(GameplayItemInstance.new(item_config1, 1))  # 物品A
	items2.append(GameplayItemInstance.new(item_config3, 1))  # 物品C
	items2.append(GameplayItemInstance.new(item_config2, 1))  # 物品B
	
	var sorted_by_name: Array[GameplayItemInstance] = sorter.sort_by_name(items2)
	assert_equal(sorted_by_name.size(), 3, "排序后数量应该不变")
	
	# 检查排序顺序（A < B < C）
	if sorted_by_name.size() >= 3:
		assert_equal(sorted_by_name[0].item_config.display_name, "物品A", "第一个应该是物品A")
		assert_equal(sorted_by_name[1].item_config.display_name, "物品B", "第二个应该是物品B")
		assert_equal(sorted_by_name[2].item_config.display_name, "物品C", "第三个应该是物品C")
	
	# 测试3: 空数组
	var empty_items: Array[GameplayItemInstance] = []
	var sorted_empty: Array[GameplayItemInstance] = sorter.sort_by_type(empty_items)
	...
	var sorted_with_null: Array[GameplayItemInstance] = sorter.sort_by_type(items_with_null)
	# null应该被过滤掉
